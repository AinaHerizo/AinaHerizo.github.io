document.addEventListener('DOMContentLoaded', function () {
    // LIST PROJET
    window.htmlProject = [
        projet1 = {
            id:"1",
            name: "LoginV1",
            image: "https://ainaherizo.github.io/LoginV1/LoginV1.webp",
            sourceGitHub: "https://github.com/AinaHerizo/LoginV1",
            sourcePage: "https://ainaherizo.github.io/LoginV1/"
        },
        projet2 = {
            id:"2",
            name: "Progress Bar Dynamique",
            image: "https://ainaherizo.github.io/progressDynamicBar/progressBar.webp",
            sourceGitHub: "https://github.com/AinaHerizo/progressDynamicBar",
            sourcePage: "https://ainaherizo.github.io/progressDynamicBar/"
        },
        projet3 = {
            id:"3",
            name:"Calculatrice",
            image: "https://ainaherizo.github.io/calculatrice/calculatrice.webp",
            sourceGitHub: "https://github.com/AinaHerizo/calculatrice",
            sourcePage: "https://ainaherizo.github.io/calculatrice/"
        },
        project4 = {
            id:"4",
            name:"Chat",
            image:"https://ainaherizo.github.io/chat/chat.webp",
            sourceGitHub: "https://github.com/AinaHerizo/chat",
            sourcePage: "https://ainaherizo.github.io/chat/"
        },
        project5 = {
            id:"5",
            name:"Filter",
            image:"https://ainaherizo.github.io/filter/filter.webp",
            sourceGitHub: "https://github.com/AinaHerizo/filter",
            sourcePage: "https://ainaherizo.github.io/filter/"
        },
    ]
    window.wordpressProject = [
        projet1 = {
            id:"???",
            name: "???",
            image: "???",
            sourceGitHub: "???",
            sourcePage: "???"
        }, 
    ]
    window.reactProject = [
        projet1 = {
            id:"???",
            name: "???",
            image: "???",
            sourceGitHub: "???",
            sourcePage: "???"
        }, 
    ]
    window.arduinoProject = [
        projet1 = {
            id:"???",
            name: "???",
            image: "???",
            sourceGitHub: "???",
            sourcePage: "???"
        }, 
    ]

    // function for disable element
    let disabledFolio = (project, id) => {
        if(project.length < 4)
        {
            id.disabled=true
            id.classList.remove("show")
        }
        else
        {
            id.disabled=false
        }
    }

    disabledFolio(window.htmlProject, document.querySelector("#html"))
    disabledFolio(window.wordpressProject, document.querySelector("#wordpress"))
    disabledFolio(window.reactProject, document.querySelector("#react"))
    disabledFolio(window.arduinoProject, document.querySelector("#arduino"))




// Affichage des projets
    // Global Variable
    const slideContainer = document.querySelector(".folio-slide > div")
    const slideHTML = document.querySelector("#htmlProject")
    const  categorieButton = document.querySelectorAll(".folio-group button")
    
    // function for creation of slide
    let slideItemCreation = (id, name, imageSrc, sourceGitHub, sourcePage) => {
        // create each element use on slide
            // Parent
            let folioItem = document.createElement("div")
            folioItem.classList.add("folio-item")
            // Child
            let imageIllustration = document.createElement("img")
            imageIllustration.classList.add("folio-item_illustration")
            imageIllustration.setAttribute("src", imageSrc)
            imageIllustration.setAttribute("alt", name)
            let itemDetail = document.createElement("div")
            itemDetail.classList.add("folio-item_detail")
            // GrandChild + their child
            let linkGitHub = document.createElement("a")
            linkGitHub.setAttribute("href", sourceGitHub)
            linkGitHub.setAttribute("title", "lien github vers le projet")
            linkGitHub.setAttribute("target", "_blank")
                let imageGitHub = document.createElement("img")
                imageGitHub.setAttribute("src", "data:image/png;base64,")
                imageGitHub.setAttribute("alt", "Logo : github")
            let linkProject = document.createElement("a")
            linkProject.setAttribute("href", sourcePage)
            linkProject.setAttribute("title", "visualisation du projet")
            linkProject.setAttribute("target", "_blank")
                let imageLinkProject = document.createElement("img")
                imageLinkProject.setAttribute("src", "./assets/images/alpha.ico")
                imageLinkProject.setAttribute("alt", "Logo : Alpha")


        // fuse element to form slide
        folioItem.appendChild(imageIllustration)
        folioItem.appendChild(itemDetail)
            itemDetail.appendChild(linkGitHub)
                linkGitHub.appendChild(imageGitHub)
            itemDetail.appendChild(linkProject)
                linkProject.appendChild(imageLinkProject)


        // Return the parent after all children are set inside the parent
        return folioItem;
    }

    // Initialisation - Suite
    htmlProject.forEach(project => {
        slideHTML.appendChild(slideItemCreation(project.id, project.name, project.image, project.sourceGitHub, project.sourcePage))
    });

    // function for switch category
    let categorieSwitch = (data, idTargeted) => {
        let slideDataTarget = document.querySelector(idTargeted)
        data.forEach(dataProject => {
            slideDataTarget.appendChild(slideItemCreation(dataProject.id, dataProject.name, dataProject.image, dataProject.sourceGitHub, dataProject.sourcePage))
        })

        // initialisation Class
        const firstItem = document.querySelector(".folio-item:first-child")
        const lastItem = document.querySelector(".folio-item:last-child")
        firstItem.classList.add("center")
        firstItem.before(lastItem)
    }
    
    // Category change
    categorieButton.forEach(button => {
        button.addEventListener("click", function(){
            slideContainer.innerHTML="";
            slideContainer.setAttribute("id", button.getAttribute("id")+'Project')
            let idProject = button.getAttribute("id")+"Project"
            let dataType = window[idProject]

            categorieSwitch(dataType, ("#"+idProject))
        })
    })
})
